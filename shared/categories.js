/**
 * 问题分类唯一数据源 — 前端与 Cloudflare Functions 共用，勿在别处重复维护。
 */
...
export const flatCategories = [
  { id: '', name: '全部' },
  ...categories.flatMap((group) => group.items.map((item) => ({ id: item.id, name: item.name }))),
]
...
export const ALLOWED_CATEGORY_IDS = categories.flatMap((g) => g.items.map((i) => i.id))

export const ALLOWED_CATEGORIES = new Set(ALLOWED_CATEGORY_IDS)

export const CATEGORY_LABELS = Object.fromEntries(
  categories.flatMap((g) => g.items.map((i) => [i.id, i.name])),
)
