...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (row.status === 'normal') {
      return json({ success: true, message: '已显示' })
    }
    const now = new Date().toISOString()
    await env.DB.prepare('UPDATE evidence SET status = ? WHERE id = ?').bind('normal', evidenceId).run()
    await env.DB.prepare(
      'INSERT INTO admin_logs (id, action, evidence_id, from_status, to_status, created_at) VALUES (?, ?, ?, ?, ?, ?)'
    ).bind(newId(), 'show', evidenceId, row.status, 'normal', now).run()
    return json({ success: true, message: '已显示' })
